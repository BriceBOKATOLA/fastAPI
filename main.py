...
def create_todo(todo: schemas.CreateArticle, session: Session = Depends(get_session)):
...
def update_todo(id: int, task: str, session: Session = Depends(get_session)):
    ...
    # check if todo item with given id exists. If not, raise exception and return 404 not found response
    if not todo:
        raise HTTPException(status_code=404, detail=f"élément avec identifiant {id} non trouvé")
